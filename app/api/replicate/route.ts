...
export async function POST(request: Request) {
  ...
  const room = req.room;
  const extraPrompt = req.extraPrompt;
  ...
  const input = {
    image,
    prompt: `A ${theme} ${room} ${extraPrompt} Editorial Style Photo, Symmetry, Straight On, Modern Living Room, Large Window, Leather, Glass, Metal, Wood Paneling, Neutral Palette, Ikea, Natural Light, Apartment, Afternoon, Serene, Contemporary, 4k`,
    a_prompt: `best quality, extremely detailed, photo from Pinterest, interior, cinematic photo, ultra-detailed, ultra-realistic, award-winning`,
  };
  console.log("input prompt:", input);

  console.log(`Extra Prompt that we are adding ::::: ${extraPrompt}`);
